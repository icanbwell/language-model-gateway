---
name: prompt-writer
description: Build FHIR server query URLs with proper parameters and environment endpoints. Use when users need to construct queries for the icanbwell FHIR server across different environments (dev, staging, client-sandbox, production). Use even if the user doesn't explicitly mention "URL building" or "query construction" but asks about accessing FHIR resources, querying patient data, or connecting to FHIR endpoints.
license: Internal use only
metadata:
  owner: icanbwell
  last_reviewed: 2024-01-15
  scope: FHIR query URL construction for icanbwell FHIR servers
---
...
The user will provide a Task, Goal, or Current Prompt
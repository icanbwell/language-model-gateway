"""
HEDIS MY 2025 - Postpartum Depression Screening and Follow-Up (PDS-E).

The percentage of deliveries in which members were screened for clinical depression
during the postpartum period, and if screened positive, received follow-up care.

Rate 1 - Depression Screening: screened during 7-84 days following the delivery
         date using a standardized instrument.
...
VALUE_SETS = load_value_sets_from_csv("PDS-E")
...
def _get_delivery_date(bundle: dict, start: date, end: date) -> date | None:
    """Find a delivery in the given date range."""
    ...
    for enc in get_resources_by_type(bundle, "Encounter"):
        ...
        if not is_date_in_range(enc_end, start, end):
            continue
...
def check_eligible_population(
    ...
    """
    Initial population: deliveries during September 8 of prior year through
    September 7 of measurement year.
    """
    evaluated: list[str] = []
    ip_start = date(measurement_year - 1, 9, 8)
    ip_end = date(measurement_year, 9, 7)

    delivery_date = _get_delivery_date(bundle, ip_start, ip_end)
...
def calculate_pds_e_measure(bundle: dict, measurement_year: int = 2025) -> dict:
    """
    Calculate PDS-E measure for a patient bundle.

    Returns a FHIR MeasureReport with two rate groups:
    1. Depression Screening (postpartum, days 7-84)
    ...
    if is_eligible and not is_excluded:
        ...
        if delivery_date:
            ...
            # Postpartum screening window: days 7-84 after delivery
            screening_start = delivery_date + timedelta(days=7)
            screening_end = delivery_date + timedelta(days=84)

            screenings = _find_screening_observations(
                bundle, screening_start, screening_end
            )
            if screenings:
                screened = True
                ...
                # Check for positive screen in 7-84 day window
                ...
                if positive_screen and first_positive_date:
                    ...
                    # Same-day negative full-length screen
    ...
    return build_multi_rate_measure_report(
        ...
        measure_abbreviation="PDS-E",
        measure_name="Postpartum Depression Screening and Follow-Up",

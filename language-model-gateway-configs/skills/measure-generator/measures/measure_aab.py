"""
HEDIS MY 2025 - Avoidance of Antibiotic Treatment for Acute Bronchitis/Bronchiolitis (AAB).

INVERSE measure: numerator = antibiotic dispensed (lower rate = better).
Episode-based: members 3 months+ with acute bronchitis who received antibiotics.
"""
...
VALUE_SETS = load_value_sets_from_csv("AAB")
...
def _find_aab_episodes(bundle: dict, measurement_year: int) -> list[tuple[dict, date]]:
    """Identify eligible acute bronchitis episodes after all filters."""
    intake_start, intake_end = _get_intake_period(measurement_year)
    birth_date = get_patient_birth_date(bundle)

    # Step 1: Outpatient/ED/Telehealth visits with acute bronchitis diagnosis
    visit_codes = all_codes(VALUE_SETS, "Outpatient, ED and Telehealth")
    bronchitis_codes = all_codes(VALUE_SETS, "Acute Bronchitis")
    if not visit_codes or not bronchitis_codes:
        return []
    ...
    for enc in get_resources_by_type(bundle, "Encounter"):
        ...
        if not resource_has_any_code(enc, visit_codes):
            ...
            if not has_visit_type:
                continue
        # Check bronchitis diagnosis
        has_dx = resource_has_any_code(enc, bronchitis_codes)
        if not has_dx:
            for reason in enc.get("reasonCode", []):
                if codeable_concept_has_any_code(reason, bronchitis_codes):
                    has_dx = True
                    break
        if not has_dx:
            continue
    ...
    if inpatient_codes:
        ...
        for enc in get_resources_by_type(bundle, "Encounter"):
            if resource_has_any_code(enc, inpatient_codes):
                d = get_encounter_date(enc)
                if d:
                    inpatient_dates.add(d)
    ...
    if comorbid_codes:
        ...
        for enc, ep_date in episodes:
            ...
            if not find_conditions_with_codes(
    ...
    if abx_codes:
        ...
        for enc, ep_date in episodes:
            ...
            if not find_medications_with_codes(
    ...
    if pharyngitis_codes or competing_codes:
        ...
        for enc, ep_date in episodes:
            ...
            has_competing = False
            if pharyngitis_codes and find_conditions_with_codes(
            ...
            if (
                not has_competing
                and competing_codes
                and find_conditions_with_codes(
...
def check_eligible_population(
    ...
    """Check if the patient has eligible acute bronchitis episodes."""
    episodes = _find_aab_episodes(bundle, measurement_year)
...
def check_numerator(bundle: dict, measurement_year: int) -> tuple[bool, list[str]]:
    ...
    INVERSE measure - numerator hit = bad outcome.
    """
    evaluated: list[str] = []
    episodes = _find_aab_episodes(bundle, measurement_year)
    if not episodes:
...
def calculate_aab_measure(bundle: dict, measurement_year: int = 2025) -> dict:
    """Calculate AAB measure and return a FHIR MeasureReport."""
    return run_measure(
        bundle=bundle,
        measure_abbreviation="AAB",
        measure_name=(

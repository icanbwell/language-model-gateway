...
and its dependency `language-model-common`.
...
> **Note:** Plugin marketplace skills and user-persisted skills are managed
> by `mcp-server-gateway`, not the gateway itself.
...
- **Singleton:** Registered as `BaseStore` in the DI container.
...
## 2. Plugin Marketplace and Skills

Plugin marketplace loading, skill caching, and user-persisted skills are
managed by `mcp-server-gateway`. The gateway accesses skills via MCP
tool endpoints exposed by mcp-server-gateway.
...
3. Eagerly load all configs           (_load_all_configs)

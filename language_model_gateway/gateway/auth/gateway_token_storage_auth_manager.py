...
class GatewayTokenStorageAuthManager(TokenStorageAuthManager):
    ...
    async def _try_register_from_mcp_json(self, auth_provider: str) -> None:
        ...
        MCP OAuth provider names follow the pattern ``mcp_oauth_{client_id}``.
        This method reads .mcp.json, finds the server entry whose OAuth
        client_id matches, and registers the provider so the callback can
        proceed.
        """
        ...
        for server_key, entry in mcp_config.mcpServers.items():
            if not entry.oauth or not entry.oauth.client_id:
                continue
            ...
            auth_config = AuthConfig(
                ...
                audience=oauth.audience or oauth.client_id,
                issuer=oauth.issuer,
